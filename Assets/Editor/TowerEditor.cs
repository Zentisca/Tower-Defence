...
public class TowerEditor : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        {
            EditorGUILayout.BeginHorizontal();
            towerPrefabs[i] = (GameObject)EditorGUILayout.ObjectField(towerPrefabs[i], typeof(GameObject), false);
        ...
        if (towerPrefabs.Count > 0 && selectedTowerIndex >= 0 && selectedTowerIndex < towerPrefabs.Count && towerPrefabs[selectedTowerIndex] != null)
        {
            ...
            if (tower != null)
            {
                ...
                EditorGUILayout.LabelField("Tower Properties (" + towerPrefabs[selectedTowerIndex].name + ")", EditorStyles.boldLabel);
                ...
                tower.bulletPrefab = (GameObject)EditorGUILayout.ObjectField("Bullet Prefab", tower.bulletPrefab, typeof(GameObject), false);

                // Add the next upgrade prefab field
                tower.nextUpgradePrefab = (GameObject)EditorGUILayout.ObjectField("Next Upgrade Prefab", tower.nextUpgradePrefab, typeof(GameObject), false);
                ...
                {
                    tower.buildableTerrainTypes[i] = EditorGUILayout.TextField($"Element {i}", tower.buildableTerrainTypes[i]);
                }
                EditorGUI.indentLevel--;
            ...
            {
                EditorGUILayout.HelpBox("Selected prefab does not have a Towers component.", MessageType.Warning);
            }
        }
    }
